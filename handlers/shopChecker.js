...
module.exports = {
	...
	async run(client) {
		try {
			let userWishlists = await client.db.collection('wishlists').find().toArray();
		
			//console.log(client.shopItemList)
			for (user of userWishlists) {
				...
				//console.log(itemListHyperlinks)
